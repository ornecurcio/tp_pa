...
def top_ctr(input, output_file):
    df = pd.read_csv(input)
    df = df.groupby(['advertiser_id', 'product_id','type']).size().reset_index(name='count')
    clicks = df[df['type'] == 'click']
    impressions = df[df['type'] == 'impression']
    # Agrupar y sumar
    clicks_sum = clicks.groupby(['advertiser_id', 'product_id']).agg({'count': 'sum'}).rename(columns={'count': 'clicks'})
    impressions_sum = impressions.groupby(['advertiser_id', 'product_id']).agg({'count': 'sum'}).rename(columns={'count': 'impressions'})
...
file2 = "files/product_views.csv"
output_file = "files/product_active_test.csv"
join_csv("2024-04-30", file1, file2,output_file)
top_product_file = "files/top_product_test_2024-04-30.csv"
top_product(output_file, top_product_file)

file3 = "files/ads_views.csv"
output_file_active_ads = "files/ads_active_test.csv"
join_csv("2024-04-30", file1, file3,output_file_active_ads)
top_ctr(output_file_active_ads, "files/top_ctr_test_2024-04-30.csv")
